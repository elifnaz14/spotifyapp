...
app = Flask(__name__)
app.secret_key = '057c414bf5ac761c579167b357b09bcc2e7f90668da8b04e'

# Spotify API credentials
CLIENT_ID = '9f51e301cf594158b80107b2b4bf54ce'
CLIENT_SECRET = 'ff7a063fc03c4086a05f1a05f511fa40'
REDIRECT_URI = 'http://127.0.0.1:5000/spotify_callback'

# ⚡ Revize edilmiş scope: recently-played eklendi
...
def login():
    ...
    <body style="background:#121212;color:white;text-align:center;font-family:sans-serif;">
        ...
        <a href="{{auth_url}}"><button style="padding:15px 30px;font-size:18px;background-color:#1DB954;border:none;border-radius:25px;color:white;cursor:pointer;">Spotify ile Giriş Yap</button></a>
    </body>
    </html>
    """, auth_url=auth_url)
...
def spotify_callback():
    ...
    # ⚡ get_access_token yerine get_cached_token kullanımı, uyarı mesajını önler
...
def dashboard():
    ...
    # ⚡ artist img + name pair, zip ile jinja hatası giderildi
    ...
    # Plotly interactive charts (div)
    bar_fig = go.Figure([go.Bar(x=artist_names, y=artist_popularity, marker_color='green')])
    bar_fig.update_layout(title="Top 5 Artist Popülerliği", plot_bgcolor='#121212', paper_bgcolor='#121212', font_color='white')
    ...
    pie_fig.update_layout(title="Top Artist Dağılımı", plot_bgcolor='#121212', paper_bgcolor='#121212', font_color='white')
    ...
    <head>
        ...
        <style>
            ...
            h2,h3 {margin:5px 0;}
            iframe {border:none;border-radius:10px;width:300px;height:80px;margin-top:10px;}
            .progress-bar {background:#333;border-radius:10px;margin-top:5px;height:15px;width:100%;}
            .progress {background:#1DB954;height:100%;border-radius:10px;}
            table {width:100%;text-align:left;margin-top:10px;}
            th, td {padding:5px;}
        </style>
    </head>
    <body>
        <h1 style="text-align:center;background:linear-gradient(90deg,#1DB954,#1ed760);-webkit-background-clip:text;color:transparent;">Spotify Dashboard</h1>
        <div class="container">
            <div class="card">
                <h2>{{display_name}}</h2>
                <p>{{email}}</p>
            ...
            <div class="card">
                ...
                {% if track_embed_url %}
                <iframe src="{{track_embed_url}}" allow="encrypted-media" allowtransparency="true"></iframe>
                <div class="progress-bar"><div class="progress" style="width:{{progress_percent}}%"></div></div>
                <p>{{'Çalıyor' if is_playing else 'Duraklatıldı'}}</p>
                {% endif %}
            </div>

            <div class="card">
                <h3>Son Dinlenen 5 Parça</h3>
                <table>
                    <tr><th>Parça</th><th>Sanatçı</th><th>Zaman</th></tr>
                    {% for name, artist, time in recent_tracks %}
                    <tr><td>{{name}}</td><td>{{artist}}</td><td>{{time}}</td></tr>
                    {% endfor %}
                </table>
            </div>
    ...
    """, display_name=display_name, email=email, profile_img=profile_img,
         artist_pairs=artist_pairs,
         track_name=track_name, track_artist=track_artist, track_embed_url=track_embed_url,
         progress_percent=progress_percent, is_playing=is_playing,
         recent_tracks=recent_tracks, bar_div=bar_div, pie_div=pie_div)
